...
describe('type-safe domain modeling lesson', () => {
  it('progresses from branding through invariants to the refactor and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'branded-email-constructor:code',
      'readonly-line-item:code',
      'brand-refund-ids:refactor',
      'domain-modeling-review:written',
    ])
  })

...
describe('end-to-end testing lesson', () => {
  it('progresses from the wait helper through the flake classifier to the review and plan', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'fix-wait-for-condition:debug',
      'classify-test-runs:code',
      'e2e-trust-review:written',
      'book-club-e2e-plan:design',
    ])
  })

...
describe('client state vs server state lesson', () => {
  it('progresses from the cache decision through the race to design and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'cache-read-decision:code',
      'fix-racing-search:react-code',
      'project-board-state-design:design',
      'server-state-review:written',
    ])
  })

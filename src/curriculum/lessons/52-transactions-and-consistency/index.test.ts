...
describe('transactions and consistency lesson', () => {
  it('progresses from the lost-update trace through optimistic writes to the retry fix and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'lost-update-trace:trace',
      'apply-optimistic-writes:code',
      'fix-transaction-retry:debug',
      'lost-update-review:written',
    ])
  })

...
describe('runtime validation and parsing lesson', () => {
  it('progresses from the predicate through parsing to the refactor and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'profile-type-predicate:code',
      'parse-profile-boundary:code',
      'restore-cart-without-casts:refactor',
      'boundary-validation-review:written',
    ])
  })
  ...
  // This lesson's core skill is surviving malformed runtime input, so the
  // unchecked starters must demonstrably fail at it: the cast-based parsers
  // throw or return wrong values where the reference returns null.
  it('shows each code starter mishandling at least one malformed input', () => {
    for (const problem of gradedProblems) {
      if (problem.kind !== 'code') {
        continue
      }

      const starterFunction = loadSolutionFunction(
        problem.starter,
        problem.functionName,
      )

      const failures = problem.tests.filter((test) => {
        try {
          const actual = starterFunction(...structuredClone(test.args))

...
`,
    ...
    },
    {
      ...
      prompt:
        'deleteInvoice is the pure handler behind `DELETE /invoices/:id`. It receives the session store, the invoice list, the request `{ headers, body }`, and the invoice id, and returns `{ status, body, invoices }`. The rules: identity comes only from the `session` cookie looked up in the store; a missing or unknown session is 401 with `{ error: { code: "unauthenticated", message: "sign in to continue" } }`. The owner or an admin may delete; anyone else, and any id that does not exist, gets 404 with `{ error: { code: "not_found", message: "invoice <id> not found" } }` so that other users\' invoices stay hidden. Success is 200 with `{ deleted: id }` and the list without that invoice. A penetration test found that sending `{ "role": "admin" }` in the body deletes anyone\'s invoice, and both refusal statuses are wrong: a missing session answers 403, and a caller who may not delete answers 401. Fix it. Example: `deleteInvoice({ s_ada: { userId: "ada", role: "member" } }, [{ id: 1, ownerId: "ada" }, { id: 2, ownerId: "grace" }], { headers: { cookie: "session=s_ada" }, body: { role: "admin" } }, 2)` should return `{ status: 404, body: { error: { code: "not_found", message: "invoice 2 not found" } }, invoices: [{ id: 1, ownerId: "ada" }, { id: 2, ownerId: "grace" }] }`.',
      estimatedMinutes: 15,

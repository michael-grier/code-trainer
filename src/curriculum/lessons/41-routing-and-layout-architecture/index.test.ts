...
describe('routing and layout architecture lesson', () => {
  it('progresses from matching through the layout fix to design and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'route-matcher:code',
      'lift-the-shell:react-code',
      'workspace-route-design:design',
      'routing-review:written',
    ])
  })

...
describe('sql fundamentals lesson', () => {
  it('progresses from the query builder through join semantics to the injection fix and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'build-invoice-list-query:code',
      'summarize-by-customer:code',
      'fix-overdue-query-builder:debug',
      'fan-out-review:written',
    ])
  })

...
describe('browser storage lesson', () => {
  it('progresses from the restore through the editor to design and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'versioned-settings-restore:code',
      'defensive-draft-editor:react-code',
      'offline-notes-design:design',
      'storage-review:written',
    ])
  })

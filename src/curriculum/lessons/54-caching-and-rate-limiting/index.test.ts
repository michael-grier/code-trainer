...
describe('caching and rate limiting lesson', () => {
  it('progresses from the cache fix through the token bucket to the plan and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'fix-cache-key-and-ttl:debug',
      'apply-token-bucket:code',
      'book-club-cache-and-limits:design',
      'cache-and-limit-review:written',
    ])
  })

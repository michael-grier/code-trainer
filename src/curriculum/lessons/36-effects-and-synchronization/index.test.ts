...
describe('effects and synchronization lesson', () => {
  it('progresses from the schedule trace through the fixes to review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'effect-schedule-trace:trace',
      'fix-stale-badge:react-code',
      'channel-viewer-cleanup:react-code',
      'effects-review:written',
    ])
  })

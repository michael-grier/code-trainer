...
describe('debugging and refactoring legacy code lesson', () => {
  it('progresses from the bisected bug through the legacy refactor to the review and plan', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'fix-late-fee-boundaries:debug',
      'refactor-legacy-shipment:refactor',
      'debugging-process-review:written',
      'strangler-pricing-plan:design',
    ])
  })

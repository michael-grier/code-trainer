...
describe('conditional types and inference lesson', () => {
  it('progresses from infer through distribution to key filtering and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'unwrap-array-value:code',
      'without-channels:code',
      'derive-searchable-keys:refactor',
      'conditional-types-review:written',
    ])
  })

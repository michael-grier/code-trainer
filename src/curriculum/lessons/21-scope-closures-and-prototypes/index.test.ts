...
describe('scope, closures, and prototypes lesson', () => {
  it('progresses from tracing through debugging to explanation', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'closure-capture-trace:trace',
      'fix-shared-loop-capture:debug',
      'resolve-with-prototypes:code',
      'closure-capture-review:written',
    ])
  })

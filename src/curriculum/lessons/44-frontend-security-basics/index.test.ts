...
describe('frontend security basics lesson', () => {
  it('progresses from the escaper through the comment fix to design and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'escape-html:code',
      'fix-comment-list-xss:react-code',
      'settings-trust-boundary-design:design',
      'trust-boundary-review:written',
    ])
  })

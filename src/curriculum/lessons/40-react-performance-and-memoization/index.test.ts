...
describe('react performance and memoization lesson', () => {
  it('progresses from the identity trace through the fixes to review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'shallow-identity-trace:trace',
      'memoize-the-report:react-code',
      'stabilize-the-callback:react-code',
      'memoization-review:written',
    ])
  })

...
describe('migrations and data evolution lesson', () => {
  it('progresses from the run planner through the backfill fix to the migration plan and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'plan-migration-run:code',
      'fix-cents-backfill:debug',
      'total-cents-migration-plan:design',
      'compatibility-window-review:written',
    ])
  })

...
describe('forms and validation lesson', () => {
  it('progresses from the validator through the form to design and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'signup-validator:code',
      'gate-the-signup-form:react-code',
      'settings-form-design:design',
      'forms-review:written',
    ])
  })

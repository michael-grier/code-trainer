...
describe('indexes and query performance lesson', () => {
  it('progresses from the prefix rule through the filter fix to the index plan and review', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'choose-index:code',
      'fix-index-hiding-filters:debug',
      'book-club-index-plan:design',
      'index-cost-review:written',
    ])
  })

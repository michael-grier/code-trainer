...
describe('system design capstone lesson', () => {
  it('progresses from the estimate through two designs to the defense', () => {
    expect(
      lesson.problems.map((problem) => `${problem.id}:${problem.kind}`),
    ).toEqual([
      'estimate-capacity:code',
      'book-club-at-scale-design:design',
      'reliability-and-rollout-plan:design',
      'three-decisions-review:written',
    ])
  })
  ...
  it('ties written and design work back to the lesson rules', () => {
    ...
    // The capstone carries two design problems, and both must meet the bar.
    const designs = lesson.problems.filter(
      (problem) => problem.kind === 'design',
    )
